...
import io.herow.sdk.detection.livemoment.model.enum.Day
...
data class TagObject(
    val tag: String,
    val locations: ArrayList<HerowQuadTreeLocation>,
    var new: Boolean = true,
    var recurrencies: HashMap<RecurrencyDay, Int> = hashMapOf()
) {

    fun addLocations(newLocations: ArrayList<HerowQuadTreeLocation>) {
        for (loc in newLocations) {
            addLocation(loc)
        }
    }

    private fun addLocation(location: HerowQuadTreeLocation) {
    ...
    private fun computeRecurrency(location: HerowQuadTreeLocation) {
